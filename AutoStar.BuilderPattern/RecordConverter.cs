...
namespace AutoStar.BuilderPattern
{
    public class RecordConverter
    {
        public ClassDeclaration LowerRecordToClass(RecordDeclaration record)
        {
            ...
            var validateMethod = new MethodDeclaration("Validate", "void")
            {
                IsStatic = false,
                IsOverride = false,
                    ...
                    new IWriteableCode[]
                        {
                            ...
                            record.RecordProperties.Where(p => p.IsOptional == false)
                                .Select(
                                    p => new Statement(
                                        $"if ({p.Name} is null) errors.Add(\"{p.Name} is null\");")))
                        .Concat(
                            new IWriteableCode[]
                            {
                                new Statement(
            ...
            var buildMethod = new MethodDeclaration("Build", record.Name)
            {
                Visibility = Visibility.Public,
                IsStatic = false,
                IsOverride = false,
                Contents = new IWriteableCode[]
                {
                    ...
                    new ConstructorCall(
                        record.Name,
                        record.RecordProperties.Select(p => p.Name).ToArray())
                }
            };
            ...
            var builderProps = record.RecordProperties
                .Select(
                    p => new Property(p.Name, p.Type)
                    {
                        HasSetter = true, DefaultValue = p.DefaultValue
                    })
                .Cast<IWriteableCode>()
                .Concat(
                    new IWriteableCode[]
                    {
                        validateMethod, buildMethod, builderException
                    })
                .ToArray();
        ...
        private MethodDeclaration BuildToStringMethod(RecordDeclaration record)
        {
            var propNames = String.Join(
                $",{Environment.NewLine}",
                record.RecordProperties.Select(p => $"$\"{p.Name} = {{{p.Name}}}\""));
            ...
            return new ClassDeclaration("ValidationException")
            {
                ...
                Contents =
                    new IWriteableCode[]
                    {
                        ...
                        {
                            HasSetter = false, Visibility = Visibility.Public
                        },
                ...
                Constructor = new Constructor.BlockConstructor("ValidationException")
                {
                    ...
                    Statements = new[]
                    {
                        new AssignmentStatement("Errors", "errors")
                    }
                }
            };
        }
